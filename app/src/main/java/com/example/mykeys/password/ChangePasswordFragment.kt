    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.buttonCreatePassword.setOnClickListener {
            ...
            if (!passwordManager.checkPassword(oldPassword)) {
                Toast.makeText(requireContext(), "Вы ввели неверный старый пароль", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }

            if (newPassword != confirmPassword) {
                Toast.makeText(requireContext(), "Новый пароль и подтверждение не совпадают", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }
